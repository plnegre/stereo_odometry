...
namespace odom
{
  Featools::Featools() : max_disparity_(0.0) {}
  ...
  void Featools::undistortKeyPoints(const vector<cv::KeyPoint> in, vector<cv::KeyPoint>& out)
  {
  ...
  void Featools::ratioMatching(const cv::Mat desc_1, const cv::Mat desc_2, const double ratio, vector<cv::DMatch> &matches)
  {
    matches.clear();
  ...
  int Featools::stereoMatchingFilter(const vector<cv::KeyPoint> ukp_1, const vector<cv::KeyPoint> ukp_2, const vector<cv::DMatch> matches, vector<cv::DMatch>& matches_filtered)
  {
